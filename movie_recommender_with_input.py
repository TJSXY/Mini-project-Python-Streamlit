...
def main():
    ...
    st.write("---")
    st.write("หรือคุณต้องการบอกความชอบของคุณเองหรือไม่?")

    # รับข้อมูลจากผู้ใช้
    user_input = st.text_input("บอกเราสิว่าคุณชอบหนังแบบไหน (เช่น 'ชอบหนังแอคชั่นที่มีการต่อสู้เยอะๆ')")

    if user_input:
        # วิเคราะห์ข้อมูลที่ผู้ใช้ป้อน
        user_preference = analyze_user_preference([user_input])
        if user_preference != "ไม่ระบุ":
            st.write(f"เข้าใจแล้ว! ดูเหมือนคุณจะชอบหนังแนว{user_preference}")
            if st.button("แนะนำหนังตามที่ฉันบอก"):
                recommendation = get_recommendation(user_preference)
                st.write(f"สำหรับหนังแนว{user_preference} เราขอแนะนำเรื่อง: {recommendation}")
        else:
            st.write("ขอโทษด้วย เรายังไม่เข้าใจความชอบของคุณ ลองบอกเราใหม่อีกครั้งได้ไหม?")

    st.write("---")
    st.write("หรือคุณอยากเลือกแนวหนังเองไหม?")

    # ให้ผู้ใช้เลือกแนวหนังเองได้ด้วย
    user_genre = st.selectbox(
        "เลือกแนวหนังที่คุณสนใจ",
